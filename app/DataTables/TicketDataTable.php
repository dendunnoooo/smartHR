...
class TicketDataTable extends DataTable
{
    ...
    public function query()
    {
        ...
        // 'My Assigned Tickets' route: only tickets assigned to current user
        if(route_is('assigned-tickets')){
            return Ticket::where('user_id', auth()->user()->id)->newQuery();
        }

    // Default ticket list: show tickets that are either
    // - created by the user and NOT yet assigned (user_id IS NULL)
    // - OR tickets assigned to the user
    return Ticket::where(function($q){
        $q->where(function($q2){
            $q2->where('created_by', auth()->user()->id)
               ->whereNull('user_id');
            })
