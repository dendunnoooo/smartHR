                            ...
                            <div class="ticket-header">
                                ...
                                @can('close', $ticket)
                                    @if($ticket->status != \App\Enums\TicketStatus::CLOSED)
                                        <form action="{{ route('tickets.close', $ticket->id) }}" method="post" style="display:inline-block; margin-left:10px;">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-outline-danger confirm-close" data-confirm="{{ __('Are you sure you want to close this ticket?') }}">{{ __('Close Ticket') }}</button>
                                        </form>
                                    @endif
                                @endcan
